page 50251 "Member Claim"
{
    ...
    layout
    {
        area(content)
        {
            ...
            part(Page; "Member Claim Subform")
            {
                ApplicationArea = All;
    ...
    actions
    {
        area(processing)
        {
            action(SendApprovalRequest)
            {
                ...
                begin
                    TESTFIELD(Remarks);
                    //ValidateAttachment;
                    IF ApprovalsMgmt.CheckMemberClaimApprovalPossible(Rec) THEN
                        ApprovalsMgmt.OnSendMemberClaimForApproval(Rec);
                    CurrPage.CLOSE;
                end;
            }
            action(CancelApprovalRequest)
            {
                ...
                var
                    ...
                    ApprovalsMgmt: Codeunit "Approvals Mgmt Ext";
                begin
                    ApprovalsMgmt.OnCancelMemberClaimApprovalRequest(Rec);
                end;
            }
            action(Approve)
            {
        ...
        area(reporting)
        {
            action(Print)
            {
                ...
                trigger OnAction()
                begin
                    MemberClaimHeader.GET("No.");
                    MemberClaimHeader.SETRECFILTER;
                    //REPORT.RUN(REPORT::"Member Claim", TRUE, FALSE, MemberClaimHeader);
                end;
            }
        }
    }
